 ...
 * - Demo mode (NOTIFY_BOARD_STATE_CHARACTERISTIC,
 * WRITE_BOARD_STATE_CHARACTERISTIC) In this mode, you can get or set whole
 * board state of the LINE Things dev board at once. Used in combination with
 * LIFF in /liff-app/linethings-dev-default of repository.
 *
 * - Command control mode / Notify switch, temperature state
 *    In this mode, all sensors and individual GPIOs can be controlled from
 * command characteristic. Easy to hack dev board from LIFF and automatic
 * communication scenario without firmware changes. Used in combination with
 * Javascript library for LIFF in /liff-app/js-control of repository. See here
 * for details.
...
#define NOTIFY_BOARD_STATE_CHARACTERISTIC_UUID \
    "e90b4b4e-f18a-44f0-8691-b041c7fe57f2"
#define WRITE_BOARD_STATE_CHARACTERISTIC_UUID \
    "4f2596d7-b3d6-4102-85a2-947b80ab4c6f"
...
#define COMMAND_RESPONSE_CHARACTERISTIC_UUID \
    "1737f2f4-c3d3-453b-a1a6-9efe69cc944f"
...
BLECharacteristic blesv_line_product =
    BLECharacteristic(blesv_line_product_uuid);
...
BLECharacteristic blech_notify_board_state =
    BLECharacteristic(blech_notify_board_state_uuid);
BLECharacteristic blech_write_board_state =
    BLECharacteristic(blech_write_board_state_uuid);
BLECharacteristic blech_version = BLECharacteristic(blech_version_uuid);
BLECharacteristic blech_command_write =
    BLECharacteristic(blech_command_write_uuid);
BLECharacteristic blech_command_response =
    BLECharacteristic(blech_command_response_uuid);
...
void bleConfigure(int power) {
    ...
    strUUID2Bytes(NOTIFY_BOARD_STATE_CHARACTERISTIC_UUID,
                  blech_notify_board_state_uuid);
    strUUID2Bytes(WRITE_BOARD_STATE_CHARACTERISTIC_UUID,
                  blech_write_board_state_uuid);
    strUUID2Bytes(VERSION_CHARACTERISTIC_UUID, blech_version_uuid);
    strUUID2Bytes(COMMAND_RESPONSE_CHARACTERISTIC_UUID,
                  blech_command_response_uuid);
...
void bleSetupServiceDevice() {
    ...
    blech_notify_board_state.setPermission(SECMODE_ENC_NO_MITM,
                                           SECMODE_NO_ACCESS);
    ...
    blech_write_board_state.setPermission(SECMODE_ENC_NO_MITM,
                                          SECMODE_ENC_NO_MITM);
    ...
    blech_command_response.setPermission(SECMODE_ENC_NO_MITM,
                                         SECMODE_NO_ACCESS);
 ...
 */
void bleWriteEvent(uint16_t conn_handle, BLECharacteristic *chr, uint8_t *data,
                   uint16_t len) {
    byte cmd = data[0];
    byte index = data[1];
    byte length = data[2];
 ...
 */
void bleIoWriteEvent(uint16_t conn_handle, BLECharacteristic *chr,
...
void i2cRequestFrom(byte address, byte length) {
    String msg = "[BLE]I2C : request from, address=" + String(address) +
...
void setup() {
    ...
    // Timer for temperature notify interval (for
    ...
    if (accel_valid) {
        Serial.print("Accel : OK");
    } else {
        Serial.print("Accel : NG");
    }
    ...
    if (memcmp(blesv_devboard_uuid, blesv_user_uuid,
               sizeof(blesv_devboard_uuid))) {
        bleSetupServiceUser();
    }
    bleStartAdvertising();
    // Set SW interrupt
...
void loop() {
    ...
    bool refresh_display =
        g_flag_display && !g_display_user_mode && !g_i2c_user_mode;
    bool notify_sensor =
        g_flag_notify && blech_notify_board_state.notifyEnabled();
    ...
    if (Bluefruit.connected()) {
        ...
        if (notify_sensor) {
            if (accel_valid) {
                notify_board_state(accel.cx, accel.cy, accel.cz, temperature,
                                   g_data_sw1, g_data_sw2);
            } else {
                notify_board_state(0, 0, 0, temperature, g_data_sw1,
                                   g_data_sw2);
            }
            g_flag_notify = 0;
        }
...
void notify_sw_handler() {
    if (blech_notify_sw.notifyEnabled()) {
        if (g_flag_sw1 &&
            (millis() - g_last_notified_sw1) > g_notify_sw.interval) {
        ...
        if (g_flag_sw2 &&
            (millis() - g_last_notified_sw2) > g_notify_sw.interval) {
...
void notify_temp_handler() {
    if (g_i2c_user_mode) {
        debugPrint(
            "User using I2C from JS. Do not work temperature notify when user "
            "using I2C from JS");
        return;
    }
...
void notify_board_state(float acc_x, float acc_y, float acc_z,
                        float temperature, int sw1, int sw2) {
    int16_t tx_frame[8] = {(int16_t) temperature * 100,
                           acc_x * 1000,
                           acc_y * 1000,
                           acc_z * 1000,
                           sw1,
